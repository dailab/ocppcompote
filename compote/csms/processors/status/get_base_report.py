...
class OCPP20GetBaseReportProcessor(GenericGetBaseReportProcessor):
    ...
    async def process(self, context: Context, request_id: int, report_base: str, custom_data: Dict = None, **kwargs):
        await super().process(context=context, request_id=request_id, report_base=report_base, **kwargs)
        result = await context.cp.send_get_base_report(request_id=request_id, report_base=report_base, custom_data=custom_data)
        return result